...
admin_panel.add("👥Foydalanuvchilarga habar yuborish")
...
@dp.message_handler(text="🏆Sovrunlar")
async def sendSovrun(message: types.Message):
    user_id = message.from_user.id
    if await check_follow(user_id):
        await message.answer(f"📚Kazbek Osiyo arsloni 23 ta referral")
        await message.answer(f"📚Sayyid Nosirxon Toʻra Kosoniy 25 ta referral")
        await message.answer(f"📚Qalbga taskin bitiklar 30 ta referral")
        await message.answer(f"💴100 000 so'm 110 ta referral")
...
@dp.message_handler(text="👥Foydalanuvchilarga habar yuborish")
...
async def all_mess(message: types.Message):
    ...
    mess = message.text
    for admins in ADMIN_ID:
        await bot.send_message(admins,  f"<a href='tg://user?id={user_id}'>{name}</a> dan Yangi 📥 xabar keldi\n Xabar: {mess}\n Id: {user_id}")
